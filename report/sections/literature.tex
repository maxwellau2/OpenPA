...
\textbf{Claude Code} \cite{anthropic2025claudecode} is Anthropic's terminal-based coding agent. Its architecture features an async-generator-based agent loop with explicit state transitions, streaming tool execution with concurrency control, and a permission system with pattern matching. The system uses deferred tool loading to avoid overwhelming the model with the full tool set on every turn, and supports context compaction to manage long conversations within context window limits.
