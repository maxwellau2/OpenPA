...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Local model quality}: While tool filtering significantly improves small model performance (reducing the tool set from 83 to 8--34 per query), local models (7--9B parameters) still struggle with complex multi-step tool-calling compared to cloud models such as Claude Sonnet or Gemini Flash.

  \item \textbf{No containerisation}: The project currently lacks Docker or Kubernetes deployment configurations, limiting reproducibility and cloud deployment readiness.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Containerised deployment}: Dockerise both the backend and frontend, with Docker Compose for local development and Kubernetes manifests for cloud deployment.
