...
async def test_search_returns_results(mcp_client, user):
    ...
    result = await mcp_client.call_tool("web_search", {
        "_user_id": user, "query": "python programming language", "num_results": 5,
    })
...
async def test_search_result_fields(mcp_client, user):
    ...
    result = await mcp_client.call_tool("web_search", {
        "_user_id": user, "query": "openai", "num_results": 3,
    })
...
async def test_search_respects_limit(mcp_client, user):
    ...
    result = await mcp_client.call_tool("web_search", {
        "_user_id": user, "query": "weather today", "num_results": 3,
    })
...
async def test_search_max_cap(mcp_client, user):
    ...
    result = await mcp_client.call_tool("web_search", {
        "_user_id": user, "query": "bitcoin", "num_results": 100,
    })
...
async def test_search_query_preserved(mcp_client, user):
    ...
    result = await mcp_client.call_tool("web_search", {
        "_user_id": user, "query": "singapore weather",
    })
