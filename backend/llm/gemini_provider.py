...
def _strip_unsupported_keys(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        return {k: _strip_unsupported_keys(v) for k, v in obj.items() if k not in UNSUPPORTED_SCHEMA_KEYS}
...
class GeminiProvider(LLMProvider):
        ...
        for t in tools:
            try:
                ...
                for pname, pval in list(props.items()):
                    if isinstance(pval, dict) and pval.get("type") == "object" and "properties" not in pval:
                        # Gemini can't handle free-form objects — treat as JSON string
                        props[pname] = {"type": "string", "description": pval.get("description", f"{pname} as JSON string")}
                ...
                logger.warning(f"Skipping tool {t.get('name', '?')} — failed to convert: {e}")
                continue
    ...
    async def chat(
        ...
        gen_config = types.GenerateContentConfig(temperature=0.4, max_output_tokens=16384)
        if system:
        ...
        try:
            ...
            if gemini_tools:
                try:
                    ...
                    pass
            logger.debug(f"Gemini request: {len(contents)} messages, {tool_count} tools")
        ...
        if not response.candidates:
            logger.warning(f"Gemini returned no candidates. Prompt feedback: {getattr(response, 'prompt_feedback', None)}")
            return LLMResponse(
        ...
        logger.debug(f"Gemini candidate finish_reason: {candidate.finish_reason}, parts: {len(candidate.content.parts) if candidate.content and candidate.content.parts else 0}")
        ...
        if not content_text and not tool_calls and gemini_tools:
            logger.warning(f"Gemini returned empty with tools — retrying without tools to force text response")
            try:
                gen_config_no_tools = types.GenerateContentConfig(temperature=0.4, max_output_tokens=16384)
                if system:
                    gen_config_no_tools.system_instruction = system
                retry_response = self.client.models.generate_content(
                    model=self.model, contents=contents, config=gen_config_no_tools,
                )
        ...
        logger.info(f"Gemini response: content={'yes' if content_text else 'EMPTY'} ({len(content_text or '')} chars), tool_calls={len(tool_calls)}")
