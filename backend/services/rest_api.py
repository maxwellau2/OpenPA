...
# Mount OAuth router
from services.oauth import router as oauth_router  # noqa: E402
...
async def save_config(
    service: str, req: ConfigRequest, user: dict = Depends(get_current_user)
):
...
async def call_tool(
    tool_name: str, arguments: dict = {}, user: dict = Depends(get_current_user)
):
...
# --- Download endpoint (YouTube etc.) ---


@app.get("/api/download/sandbox/{download_id}")
async def download_sandbox_file(
    download_id: str, user: dict = Depends(get_current_user)
):
    """Serve a file generated by the sandbox (CSV, JSON, etc.)."""
    import os
    from tools.sandbox import get_sandbox_file

    info = get_sandbox_file(download_id)
    if not info:
...
async def download_file(download_id: str, user: dict = Depends(get_current_user)):
    """Serve a downloaded file (e.g., YouTube video)."""
    import os
    from tools.youtube import get_download_info
...
async def list_jobs(user: dict = Depends(get_current_user)):
    ...
    from tools.scheduler import _jobs

    user_jobs = [
        {"job_id": jid, **{k: v for k, v in job.items() if k != "user_id"}}
...
async def cancel_job(job_id: str, user: dict = Depends(get_current_user)):
    ...
    from tools.scheduler import _jobs, _running_tasks
...
async def get_llm_config(user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "configured_providers": [
            k.replace("_api_key", "")
            for k in creds
            if k.endswith("_api_key") and creds[k]
        ],
    }
...
async def save_llm_config(
    req: LLMConfigRequest, user: dict = Depends(get_current_user)
):
    ...
    logger.info(
        f"LLM config saved: provider={existing.get('default_provider')}, model={existing.get('default_model')}"
    )
    return {"status": "saved"}
...
async def chat_stream(req: ChatRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        async def error_gen():
            yield AgentEvent("error", {"error": err_msg}).to_sse()

        return StreamingResponse(error_gen(), media_type="text/event-stream")

    conv_id = await get_or_create_conversation(
        user["id"], req.conversation_id, req.message
    )
